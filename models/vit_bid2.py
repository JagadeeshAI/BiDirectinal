...
class VisionDectector(nn.Module):
    """Vision Transformer with support for object detection head"""
    ...
    def forward_features(self, x):
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if hasattr(self, "config") and hasattr(self.config, "vpt_on") and self.config.vpt_on:
                eee = self.embeddings[idx].expand(B, -1, -1)
                x = torch.cat([eee, x], dim=1)

            if hasattr(self, "config") and hasattr(self.config, "ffn_adapt") and self.config.ffn_adapt:
                if idx in self.adapt_pos:
                    pos = self.adapt_pos.index(idx)
                    if hasattr(self, "use_block_weight") and self.use_block_weight and idx in self.specfic_pos:
    ...
    def forward(self, images, targets=None):
        ...
        if self.training and targets is not None:
            ...
            pred_boxes = bbox_preds.view(-1, 4)             # [B, 4]
            pred_logits = class_logits.view(-1, self.num_classes)  # [B, C]
            tgt_boxes = torch.cat(all_gt_boxes, dim=0)      # [B, 4]
            tgt_labels = torch.cat(all_gt_labels, dim=0)    # [B]
            ...
            outputs = []
            for i in range(B):
                scores = torch.softmax(class_logits[i], dim=-1)        # [num_classes]
