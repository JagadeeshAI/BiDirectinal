...
from uitlis import get_model, print_parameter_stats
...
def validate(model, loader, device, logger):
    ...
    for images, targets in loader:
        ...
        # Always get loss dict by calling model(images, targets)
        outputs = model(images, targets)
        if isinstance(outputs, dict):
            loss_cls = outputs.get("loss_cls", torch.tensor(0.0, device=device))
            loss_bbox = outputs.get("loss_bbox", torch.tensor(0.0, device=device))
            ...
            # If model returns only predictions, compute loss manually
            preds = outputs
            # You must implement your own loss calculation here if needed
            pass
